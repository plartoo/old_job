...
files = Dir.glob("log/*.log")

size_info = nil
files.each do |resultFile|
...
f = File.open(resultFile,'r').each{ |line|
  ...
  if line =~ /INVALID\sSIZE:/
    m = /.*value:(.+?)\sis.*dept:(.*?),\sclothing_type:\s\{:group=>.*?(\w+).*?:bm=>.*?(\d+)/.match(line)
    if m.nil?
      m = /.*value:(.+?)\sis.*dept:(.*?),\sclothing_type:\s\{:bm=>.*?(\d+).*?:group=>.*?(\w+)/.match(line).to_a
      t = m[3]
      m[3] = m[4]
      m[4] = t
    end
    #/.*value:(.+?)\sis.*dept:(.*?),\sclothing_type:\s\{:bm=>(.*?), :group=>:(\w+)\}/.match(line)
  ...
  if line =~ /URL:/
    ...
    if !link.nil? & !size_info.nil?
      if a[size_info[2].strip][size_info[3].to_s.strip][size_info[1].to_s.strip].empty?
        a[size_info[2].strip][size_info[3].to_s.strip][size_info[1].to_s.strip] = []
        a[size_info[2].strip][size_info[3].to_s.strip][size_info[1].to_s.strip].push(link[1]).uniq
      elsif a[size_info[2].strip][size_info[3].to_s.strip].nil?
        a[size_info[2].strip][size_info[3].to_s.strip][size_info[1].to_s.strip] = []
        a[size_info[2].strip][size_info[3].to_s.strip][size_info[1].to_s.strip].push(link[1]).uniq
      else
        a[size_info[2].strip][size_info[3].to_s.strip][size_info[1].to_s.strip].push(link[1]).uniq
...
a.keys.each{|k1|
  ...
  a[k1].keys.each{|k2|
    puts ","+k2.to_s
    a[k1][k2].keys.sort_by{|key|  -1 * a[k1][k2][key].uniq.size }.each {|k3|
      puts ",,"+k3.to_s+","+a[k1][k2][k3].uniq.size.to_s
       count = 0
