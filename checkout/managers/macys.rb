...
module Checkout
  class Macys < MechanizeCheckoutManager

    HOMEPAGE = "http://www.macys.com"

    CHECKOUT_FLOW_URL = "https://www.macys.com/checkoutswf/checkout-webflow?execution="

    BILLING_PAGE_TITLE = "Billing Information"
    ...
    def do_add_to_bag
      ...
      agent_get(:go_to_shopping_bag, {:headers => {}, :no_verify => [:custom_assert]})

      # we need to custom assert after go_to_profile_creation post request because Macy's items are easily out-of-stock
      post_and_validate(:go_to_profile_creation)
    ...
    GO_TO_SHOPPING_BAG_URL = "http://www1.macys.com/bag/index.ognc"
    GO_TO_SHOPPING_BAG_PAGE_TITLE = "Shopping Bag - Macy's"

    ### These time limits calls overrides the DEFAULT_TIME_LIMIT definied in mechanize_checkout_manager
    TIME_LIMIT_FOR_DO_FETCH_DETAILS = 15
    ...
    def shipping_method_option_exists?
      @page.search('div.ShippingOption_shippingMethod').any?
    end
    ...
    def item_description
      @page.search('span.OrderReview_content span')[0].text.strip rescue nil
    end
    ...
    def fetch_order_total_data
      item_price, shipping_cost, tax = @page.search('table#orderSummaryRightSidebar tr').first(3).map{|i| CheckoutUtils.price_str_to_decimal(i.text) rescue nil}

      order_total = page_search_price('tr.orderSummaryTotalCell2')
    ...
    def order_number
      @page.search("div[align='center'] div[align='left'] span")[0].text.strip rescue nil
    end

    def order_info_detail
      @page.search("div[align='center'] div[align='left']").to_s rescue nil
    end
    ...
    def remove_item_from_bag
      ...
      # unique to Macy's: get upc_id
      @upc_id = @page.content.match(/upc_ID=(\d+)/)[1] rescue nil
    ...
    ADD_TO_BAG_URL = "http://www1.macys.com/bag/addto.ognc"

    def assert_for_add_to_bag(parameters_used)
      error_div = @page.search('td.errorText')
      if error_div.any?
    ...
    def assert_for_go_to_profile_creation(parameters_used)
      error_div = @page.search('div.errorText')
      if error_div.any?
        error_message = error_div.text.strip rescue 'Unknown Error.'
        raise_out_of_stock({:add_to_bag => "Proceeding from shopping bag failed for: #{error_message}"},
                           {:parameters_used => parameters_used})
      end
    end

    GO_TO_SHIPPING_INFO_URL = "https://www.macys.com/signin/index.ognc"
    ...
    def go_to_shipping_info_parameters
      {
        ...
        'CHECKOUT_BUTTON.x' => '88',
        'CHECKOUT_BUTTON.y' =>	'13',
        'fromCheckout' => 'fromCheckout',
        'fromPage' => 'null',
      }
    end

    SKIP_PROFILE_CREATION_AND_GO_TO_SHIPPING_INFO_URL = "http://www1.macys.com/bag/index.ognc"
    ...
    def skip_profile_creation_and_go_to_shipping_info_parameters
      {
        ...
        "QuantityMyself99|990___1___#{@upc_id}"	=> '1',
        'bagUrl' => '',
        "colorMyself99|990___1___#{@upc_id}"	=> '',
        'currencyCode' => '',
        'exchangeRateId' => '',
        'itemstate'	=> '0',
        'landedCostCoefficientId' => '',
        'shippingCountryCode' => '',
        "sizeMyself99|990___1___#{@upc_id}"	=> '',
        "typeMyself99|990___1___#{@upc_id}"	=> '',
      }
    end

    GO_TO_PROFILE_CREATION_URL = "http://www1.macys.com/bag/index.ognc"
    GO_TO_PROFILE_CREATION_PAGE_TITLE = "Sign In - Macy's"

    def go_to_profile_creation_parameters
      {
        'Action' => '',
        'CHECKOUT_BUTTON' => 'CHECKOUT_BUTTON',
        'CHECKOUT_BUTTON.x' => '21',
        'CHECKOUT_BUTTON.y' => '11',
        'PromoCode' => '',
        "QuantityMyself99|990___1___#{@upc_id}" => '1',
        "RecipientMyself99|990___1___#{@upc_id}" => "RecipientMyself99|990___1___#{@upc_id}",
        "UniqueIDMyself99|990___1___#{@upc_id}" => "Myself99|990___1___#{@upc_id}",
        'bagUrl' => '',
        'cmio_productid' => '',
        "colorMyself99|990___1___#{@upc_id}" => '',
        'currencyCode' => '',
    ...
    def assert_for_shipping_address(parameters_used)
      ...
      if @page.search('div.generalError').any?
        error_hash[:main_message] = @page.search('div.generalError').text.strip + "\n" rescue nil

        @page.search('label.errorText').each do |element|
    ...
    BILLING_ADDRESS_PAGE_TITLE = "Review Your Order"
    alias_method :billing_address_url, :checkout_flow_trial_number_url

    def assert_for_billing_address(parameters_used)
      error_hash = {}
      if @page.search('div.generalError span').any?
        error_hash[:main_message] = @page.search('div.generalError span').text.strip + "\n" rescue nil
    ...
    def billing_address_parameters
      phone_num = split_phone_number_in_three(@billing_phone)
      {
        ...
        'useShippingAsBilling' => 'Gift',
      }
    end

    REMOVE_FROM_BAG_URL = "http://www1.macys.com/bag/index.ognc"

    def remove_from_bag_parameters
      {
        'Action' => "DELETE_LINKMyself99|990___1___#{@upc_id}",
        'PromoCode' => '',
        "QuantityMyself99|990___1___#{@upc_id}" => '1',
        "RecipientMyself99|990___1___#{@upc_id}" => "RecipientMyself99|990___1___#{@upc_id}",
        "UniqueIDMyself99|990___1___#{@upc_id}" => "Myself99|990___1___#{@upc_id}",
