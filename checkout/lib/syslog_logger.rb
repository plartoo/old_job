...
class SyslogLogger
  ...
  def self.make_methods(meth)
    prefix = [:warn,:error,:fatal].include?(meth) ? "#{meth.to_s.upcase}: " : ''
    eval <<-EOM, nil, __FILE__, __LINE__ + 1
      def #{meth}(message = nil)
        return true if #{LOGGER_LEVEL_MAP[meth]} < @level
        SYSLOG.#{LOGGER_MAP[meth]} '#{prefix}'+clean(message || yield)
        return true
      end
...
#from /usr/include/sys/syslog.h
  ...
  LOG_LOCAL7 =     (23<<3)
  LOG_USER =        (1<<3)
  def initialize(program_name = 'rails', facility=LOG_USER)
    @level = Logger::DEBUG

    return if defined? SYSLOG
    self.class.const_set :SYSLOG,  Syslog.open(program_name,OPTION_PIDS,facility)
  end
